...
class AcketViewController extends Controller{
  public function index($id){
    ...
    $svg = '<div width="400" style="height: 400px; overflow:scroll; overflow-y:scroll;"><svg height="3000" width="1600"><g>';
    $svg = $svg.'<rect x="0" y="0" width="1600" height="3000" style="fill:#efefef;" />';
    ...
    if(($num_of_players & ($num_of_players-1))!=0){
      ...
      */
      ...
      if(sizeof($brackets['depth0'])<=sizeof($brackets['depth1'])){
        ...
        $numOfSecondRoundGames = sizeof($second_round);
        $x_cord = 200;
        $y_cord = (550-($numOfSecondRoundGames*50+($numOfSecondRoundGames-1)*65))/2;
        if($y_cord<0) $y_cord = 0;
        foreach($second_round as $game){
          $p0 = $game['p0'];
          $p1 = $game['p1'];
