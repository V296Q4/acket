	...
	public function UpdateAcket(){
		...
		if($tournamentId !== null && Auth::check()){
			...
			if(Input::get("4") !== null){//log match
				...
				$loser = DB::table('participants')->where('tournamentId', $tournament['id'])->where('gameId', $gameId)->where('participantSide', $loserSide)->where('depthId', $winnerDepthId)->first();
				$loserUpdate = array();
				$loserUpdate['gameStatus'] = 1;
				$loserUpdate['tournamentStatus'] = 1;
				
				$updateCount = DB::table('participants')->where('tournamentId', $tournament['id'])->where('participantId', $loser['participantId'])->update($loserUpdate);
				...
				$winnerUpdate['depthId'] = $winnerDepthId + 1;
				...
				$maxDepth = (int)(ceil(log($playerCount, 2)));
				//dd($maxDepth);
				...
				if($activePlayersCount <= 0){//winner is last winner of round - round over, reset game statuses
					$winnerUpdate['gameStatus'] = 0;
					//dd($winnerDepthId . ' vs max of ' . $maxDepth);
					if($winnerDepthId == $maxDepth - 1){//won the finals
					...
					if($winnerDepthId == $maxDepth - 2){//semi finals done, start finals
						$tournamentStatus = 2;
					}
				...
				return Redirect::to('/acket/' . $tournamentId . '?pc=' . $activePlayersCount);
			}
